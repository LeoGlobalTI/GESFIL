...
const StaffController: React.FC<{ 
  ...
  }, [user.assignedStationId, isAdmin, activeStationId]);

  useEffect(() => {
    if (isAdmin && !activeStationId && stations.length > 0) {
      const firstActive = stations.find(s => s.active);
      if (firstActive) {
        setActiveStationId(firstActive.id);
      }
    }
  }, [isAdmin, activeStationId, stations]);
...
const App: React.FC = () => {
  const { 
    state, loading, addTicket, updateTicketStatus, resetSystem, login, logout, 
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f8fafc] space-y-4">
        ...
        <p className="text-slate-500 font-bold uppercase tracking-widest text-xs animate-pulse">Cargando GESFIL...</p>
      </div>
    );
  }
